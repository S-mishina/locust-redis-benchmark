...
class RedisTaskSet(TaskSet):
    ...
    def cache_scenario(self):
        ...
        if random.random() < float(hit_rate):
            ...
            result = locust_redis_get(self, self.user.environment.cache_conn, key, "default")
            if result is None:
                value = generate_string(os.environ.get("VALUE_SIZE"))
                ttl = int(os.environ.get("TTL"))
                locust_redis_set(self, self.user.environment.cache_conn, key , value , "default", ttl)
        else:
            hash_key = hashlib.sha256(str(time.time_ns()).encode()).hexdigest()
            ttl = int(os.environ.get("TTL"))
            result = locust_redis_get(self, self.user.environment.cache_conn, hash_key, "dummy")
            if result is None:
                value = generate_string(os.environ.get("VALUE_SIZE"))
                locust_redis_set(self, self.user.environment.cache_conn, hash_key , value , "dummy", ttl)
